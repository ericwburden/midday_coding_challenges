...
impl From<&Vec<(char, Vec<(char, usize)>)>> for PipeGraph {
    fn from(value: &Vec<(char, Vec<(char, usize)>)>) -> Self {
        let mut out_graph = PipeGraph::default();

        for (start, destinations) in value {
            let start_node = Node::from(*start);
            for (end, cost) in destinations {
                let end_node = Node::from(*end);
                out_graph.insert_edge(&start_node, &end_node, *cost);
                out_graph.insert_edge(&end_node, &start_node, *cost);
            }
        }
...
impl PipeGraph {
    ...
    /// Return the Nodes connected to the given Node
    ...
    fn min_spanning_tree(&self, node: Node) -> PipeGraph {
        let mut out_graph = PipeGraph::default();
        let mut visited = HashSet::new();
        let mut heap = BinaryHeap::new();
        if self.0.contains_key(&node) {
            heap.push((Reverse(0), &node, &node)); // Sort cost, start, end
        }

        while let Some((cost, start, end)) = heap.pop() {
            if !visited.insert(end) { continue; }

            out_graph.insert_edge(start, end, cost.0);
            for neighbor in self.get_neighbors(end) {
                let neighbor_edge_cost = self.get_edge_cost(end, neighbor);
                heap.push((Reverse(neighbor_edge_cost), end, neighbor));
            }
        }
...
mod tests {
    ...
    fn should_find_minimum_cost_for_simplest_map() {
        ...
        let result = pipe_graph.min_spanning_tree(Node::from('S'));
    ...
    #[test]
    fn should_find_minimum_cost_for_another_example() {
        let pipes = vec![
            ('S', vec![('A', 8), ('B', 10)]),
            ('A', vec![('C', 2)]),
            ('B', vec![('C', 1)]),
        ...
        assert!(!result.has_edge(&Node::from('S'), &Node::from('C')));
        assert!(result.has_edge(&Node::from('A'), &Node::from('C')));
        assert!(result.has_edge(&Node::from('C'), &Node::from('B')));
